...
export interface RouletteState{version:1;streamerId:string;boxes:RouletteBox[];pending:PendingBalance[];history:RouletteHistory[];seenEventIds:string[]}

const empty=():RouletteState=>({version:1,streamerId:'',boxes:[],pending:[],history:[],seenEventIds:[]});
...
export class RouletteEngine{
  state:RouletteState;
  constructor(initial?:RouletteState){this.state=initial??empty()}
  ...
  createBox(input:{title?:string;code?:string;goal:number}){
    ...
    this.state.boxes.push(box);this.log('box_created',{title,code,goal:box.goal},box.id);return box;
  }
  archiveBox(id:string){const box=this.box(id);if(box.status==='collecting'&&box.collected)for(const share of box.shares)this.addPending(share);box.status='archived';this.log('box_archived',{title:box.title},id);return box}
