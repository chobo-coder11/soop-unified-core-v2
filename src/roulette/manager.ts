...
export class RouletteManager extends EventEmitter{
  ...
  createBox(v:{title?:string;code?:string;goal:number}){const box=this.engine.createBox(v),rolled=this.engine.rollPendingInto(box.id);if(rolled.winner)this.changed('spin',{box,result:rolled,event:{nickname:'이월 별풍선',amount:rolled.allocated,message:'',kind:'rollover'}});else this.changed('box_created',{box,rolledOver:rolled.allocated});return box}
  ...
  donation(e:CanonicalEvent){if(!isCountableStarBalloon(e))return;const extra=e.donation!.extra as{ttsData?:unknown;message?:unknown}|undefined,message=e.message??(typeof extra?.ttsData==='string'?extra.ttsData:typeof extra?.message==='string'?extra.message:'');const amount=e.donation!.amount!;const result=this.engine.processDonation({eventId:e.id,userId:e.user?.id,nickname:e.user?.nickname,amount,message});if(result.accepted)this.changed(result.winner?'spin':'donation',{result,event:{userId:e.user?.id,nickname:e.user?.nickname,amount,message,kind:e.donation!.kind}})}
