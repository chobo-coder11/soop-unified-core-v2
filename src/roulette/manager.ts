...
export class RouletteManager extends EventEmitter{
  ...
  donation(e:CanonicalEvent){if(!isCountableStarBalloon(e))return;const extra=e.donation!.extra as{ttsData?:unknown;message?:unknown}|undefined,message=e.message??(typeof extra?.ttsData==='string'?extra.ttsData:typeof extra?.message==='string'?extra.message:'');const amount=e.donation!.amount!;const result=this.engine.processDonation({eventId:e.id,userId:e.user?.id,nickname:e.user?.nickname,amount,message});if(result.accepted)this.changed(result.winner?'spin':'donation',{result,event:{nickname:e.user?.nickname,amount,message,kind:e.donation!.kind}})}
