...
export class EventDeduplicator{
  ...
  private entryCount=0;private lastPruneAt=0;
  constructor(private ttlMs=2500,private maxEntries=20000){}

  accept(e:CanonicalEvent){
    const now=Date.now();if(this.entryCount>this.maxEntries||now-this.lastPruneAt>Math.min(1000,Math.max(250,this.ttlMs/2))){this.prune(now);this.lastPruneAt=now}const fp=fingerprintEvent(e);e.fingerprint=fp;
