...
interface Entry{conn:NativeChatConnection;mirror?:ReindeerChatMirror;refs:number;manual:boolean;idle?:NodeJS.Timeout;createdAt:string;lastEventAt?:string;events:number}
export class ConnectionPool extends EventEmitter{
 ...
 async acquire(id:string,manual=false){
  ...
  if(!e){
   const conn=new NativeChatConnection(id,this.providers.native.directHttp(),{maxBackoffMs:this.cfg.chatMaxBackoffMs,allowInsecureTls:this.cfg.allowInsecureTls,pingIntervalMs:this.cfg.chatPingIntervalMs,livenessTimeoutMs:this.cfg.chatLivenessTimeoutMs,joinTimeoutMs:this.cfg.chatJoinTimeoutMs,offlinePollMs:this.cfg.chatOfflinePollMs});
   const mirror=this.cfg.enableReindeerEvents?new ReindeerChatMirror(id,this.cfg.chatMaxBackoffMs):undefined;e={conn,mirror,refs:0,manual,createdAt:new Date().toISOString(),events:0};this.entries.set(id,e);
 ...
 private onEvent(entry:Entry,e:CanonicalEvent){entry.events++;entry.lastEventAt=e.receivedAt;this.rawStore.push(e);this.metrics.incLabel('soop_events_ingested_total',{type:e.type,source:e.source});if(!this.dedup.accept(e)){this.metrics.incLabel('soop_events_deduplicated_total',{source:e.source});return}const incident=this.drift.observeEvent(e);if(incident){this.rawStore.recordIncident(incident);this.metrics.incLabel('soop_protocol_drift_incidents_total',{reason:incident.reason,severity:incident.severity});this.emit('drift-incident',incident)}this.metrics.incLabel('soop_events_total',{type:e.type,source:e.source});this.emit('event',e)}
 ...
 list(){return[...this.entries.entries()].map(([streamerId,e])=>({streamerId,state:e.conn.state,realtimeProviders:{native:e.conn.state,reindeer:e.mirror?.state??'disabled'},refs:e.refs,manual:e.manual,createdAt:e.createdAt,lastEventAt:e.lastEventAt,events:e.events,diagnostics:e.conn.diagnostics()}))}
