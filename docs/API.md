...
`/v1/providers`는 circuit state와 adaptive reliability(`dynamicWeight`, success/disagreement EWMA, latency)를 함께 제공합니다.
...
서버 hello protocol은 `3`입니다. downstream WS에는 client cap, max payload, idle heartbeat, subscription cap, backpressure drop protection이 적용됩니다.
...
동일 스트리머를 여러 API client가 구독해도 내부 Native SOOP WebSocket은 connection pool에서 공유됩니다.
